...
export function generateInvoicePDF(data: InvoiceData) {
  ...
  const amountTableY = (doc as any).lastAutoTable.finalY + 10
  
  const amountDetails: any[][] = []
  ...
  doc.text('This is a computer-generated receipt and does not require a signature.', 105, footerY + 12, { align: 'center' })
  doc.text('For any queries, please contact us at info@probodyline.com', 105, footerY + 17, { align: 'center' })
  
  // Save the PDF
  const fileName = `Invoice_${data.invoiceNumber}_${data.memberName.replace(/\s+/g, '_')}.pdf`
  doc.save(fileName)
