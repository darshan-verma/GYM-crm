...
export function StaffForm({ userRole, roles: initialRoles }: StaffFormProps) {
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-2 p-3 border rounded-lg bg-gray-50 max-h-48 overflow-y-auto">
										{Object.values(Permission).map((perm) => (
											<div key={perm} className="flex items-center space-x-2">
												...
												/>
												<Label htmlFor={`new-role-${perm}`} className="text-sm font-normal">
													{perm.replace(/_/g, " ")}
												</Label>
											</div>
										))}
