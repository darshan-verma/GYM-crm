...
export async function GET(request: NextRequest) {
  try {
    ...
    const paymentModeBreakdown = payments.reduce((acc: any, payment) => {
    ...
    const summaryData = [
      ...
      { 'Metric': 'Average Payment (₹)', 'Value': payments.length > 0 ? (totalAmount / payments.length).toFixed(2) : '0' },
    ...
    Object.entries(paymentModeBreakdown).forEach(([mode, data]: [string, any]) => {
      summaryData.push({
        'Metric': `${mode} - Count`,
