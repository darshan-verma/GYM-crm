...
export default function GymProfilesMembershipsTable({
  profiles,
  total,
  currentPage,
  totalPages,
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    startTransition(() => {
      router.push(`/memberships?${params.toString()}`)
    })
  }
              ...
              <TableRow key={p.id} className="hover:bg-muted/50">
                ...
                <TableCell className="text-right">
                  <Button asChild size="sm" variant="outline">
                    <Link
                      href={`/memberships?gymProfileId=${encodeURIComponent(
                        p.id
                      )}&page=1&limit=${encodeURIComponent(String(limit))}`}
                    >
                      <Info className="h-4 h-4 mr-2" />
                      View details
                    </Link>
                  </Button>
                </TableCell>
