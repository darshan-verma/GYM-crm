...
import { formatDate, formatCurrency, formatPhoneNumber, formatDuration } from '@/lib/utils/format'
...
export default async function MemberDetailPage({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Expiry Alert */}
      {activeMembership && new Date(activeMembership.endDate) < new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) && (
        <Card className="border-orange-200 bg-orange-50">
          <CardContent className="flex items-center gap-3 pt-6">
            <AlertCircle className="w-5 h-5 text-orange-600" />
            <div>
              <p className="font-medium text-orange-900">Membership Expiring Soon</p>
            ...
            {member.phone && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="font-medium">{formatPhoneNumber(member.phone)}</p>
                </div>
              </div>
            )}
            {member.email && (
              ...
              <>
                <div>
                  ...
                  {member.emergencyContact && (
                    <p className="text-sm mt-1">{formatPhoneNumber(member.emergencyContact)}</p>
                  )}
                </div>
              </>
            )}
          ...
          {activeMembership && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Current membership plan details</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    <p className="text-sm text-muted-foreground">Plan</p>
                    <p className="text-lg font-semibold">{activeMembership.plan.name}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Amount Paid</p>
                    <p className="text-lg font-semibold">{formatCurrency(Number(activeMembership.finalAmount))}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Start Date</p>
                    <p className="font-medium">{formatDate(activeMembership.startDate)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">End Date</p>
                    <p className="font-medium">{formatDate(activeMembership.endDate)}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                      ...
                      {recentAttendance.map((record) => (
                        ...
                        >
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              Check-in: {new Date(record.checkIn).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                              {record.checkOut && (
                                <> • Check-out: {new Date(record.checkOut).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</>
                              )}
                            </p>
                          </div>
                          {record.duration && (
                ...
                <CardContent>
                  {member.workoutPlans.length === 0 ? (
                    <div className="text-center py-8">
                      <p className="text-muted-foreground mb-4">No workout plans assigned</p>
              ...
              <Card>
                ...
                <CardContent>
                  {member.dietPlans.length === 0 ? (
                    <div className="text-center py-8">
                      <p className="text-muted-foreground mb-4">No diet plans assigned</p>
                      <Button size="sm" asChild>
                        ...
                        <div key={plan.id} className="p-3 border rounded-lg">
                          ...
                          {plan.totalCalories && (
                            <p className="text-sm mt-2">
                              <span className="font-medium">{plan.totalCalories}</span> calories/day
                            </p>
                          )}
                        </div>
                      ))}
