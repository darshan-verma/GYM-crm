...
export async function getTopRevenueByPlan() {
  ...
  // Aggregate by plan
  const planRevenue = memberships.reduce((acc: any, membership) => {
...
export async function getAttendanceByDayOfWeek() {
  ...
  const dayDistribution = attendances.reduce((acc: any, record) => {
...
export async function getOverallStats() {
  ...
  const revenueGrowth =
    lastRevenue > 0 ? Math.round(((currentRevenue - lastRevenue) / lastRevenue) * 100) : 0
