...
export async function GET(request: Request) {
  try {
    ...
    for (const membership of expiringMemberships) {
      ...
      try {
        const expiryDate = new Date(membership.endDate).toLocaleDateString('en-IN')
        ...
        console.error(`Failed to send notification for member ${membership.memberId}:`, error)
        errors.push({
          memberId: membership.memberId,
  ...
  try {
    ...
    return NextResponse.json({
      ...
      memberships: expiringMemberships.map(m => ({
        ...
        daysRemaining: Math.ceil((new Date(m.endDate).getTime() - today.getTime()) / (1000 * 60 * 60 * 24))
      }))
    })
  } catch (error) {
