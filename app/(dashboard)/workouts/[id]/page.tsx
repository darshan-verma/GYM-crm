...
import { ArrowLeft, User, Calendar, Target, Dumbbell, Clock, Edit } from 'lucide-react'
...
export default async function WorkoutPlanDetailPage({
  ...
  const plan = await getWorkoutPlan(id) as any
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card>
          <CardHeader>
            <CardTitle className="text-sm font-medium text-gray-600">Difficulty</CardTitle>
          </CardHeader>
          <CardContent>
            {plan.difficulty && (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Exercises ({(plan.exercises as any[]).length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {(plan.exercises as any[]).map((exercise, index) => (
              <div key={index} className="p-4 border rounded-lg hover:border-blue-300 transition-colors">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h4 className="font-semibold text-lg">{index + 1}. {exercise.name}</h4>
                    {exercise.notes && (
        ...
        <CardContent>
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <Link href={`/members/${plan.member.id}`} className="font-medium hover:text-blue-600">
                {plan.member.name}
              </Link>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Member ID:</span>
              <span className="font-medium">{plan.member.membershipNumber}</span>
            </div>
            {plan.member.email && (
              <div className="flex justify-between">
